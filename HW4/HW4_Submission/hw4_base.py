...
def create_parser():
    ...
    parser.add_argument('--input_shape', nargs=3, type=int, default=[256, 256, 26], help='Input shape as a sequence of three ints')
    ...
    parser.add_argument('--skip_connections', action='store_true', help='Include skip connections for U-Net architecture')
...
def generate_fname(args, params_str):
    ...
    # Conv configuration
    conv_size_str = '_'.join(str(x) for x in args.kernelSize)
    conv_filter_str = '_'.join(str(x) for x in args.n_filters)
    pool_str = '_'.join(str(x) for x in args.pool_size)
